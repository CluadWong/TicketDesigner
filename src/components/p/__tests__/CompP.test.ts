...
describe('CompP', () => {
  it('渲染 <p> 含 {field} 占位', () => {
    const comp: PComponent = { id: 'p1', type: 'p', field: 'hello' }
  ...
  it('占位含 HTML 字符应被 Vue 转义', () => {
    const comp: PComponent = { id: 'p1', type: 'p', field: '<script>alert(1)</script>' }
    const wrapper = mount(CompP, { props: { comp } })
    // Vue 默认插值会转义 HTML 字符
