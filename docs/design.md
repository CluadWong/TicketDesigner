...
所有 Page、Grid、Row、Cell 和组件都必须有稳定 ID。设计器加载 Schema 后建立：
...
- repeatable=true 时，预览数据可以扩展实际行数。
...
- repeatable Table 读取 data[field] 数组。
...
动态明细报表后续支持 flow：普通 Grid 作为不可拆分块，repeatable Table 可以按完整行切分并重复表头。
...
2. 标题、四个基本信息行和工作任务行结构正确。
3. 单行高度一致，工作任务高度等于 5 个基础行高。
4. 左侧“工作任务”竖排，右侧 Table 包含表头和 4 行输入格。
5. 所有输入位置都有独立 field。
6. Schema 保存、重新加载后效果不变。
7. 所有节点仍可被选中、移动和配置。
8. 填入测试数据后能回写 data。
9. A4 打印截图与参考 HTML/图片接近。
