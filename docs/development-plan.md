...
- **列宽是否支持 `%` 单位（2026-09-08 提出，待决）**：当前真相——渲染层 `GridSchemaNode.track()` 对字符串**原样透传**到 CSS `grid-template-columns`，故 `%` 在渲染上本就能生效；但**三道关卡把 `%` 判非法**：① 类型 `GridTrackV2 = number | ${number}fr | "auto"` 不含百分比；② `isValidTrack` 只认 `auto`/正数/`${n}fr`，`30%` 会报 `INVALID_CELL_WIDTH`；③ `useSchemaEdits.parseColumnWidth` 遇 `%` 返回 `"INVALID"` → `updateGridColumnWidth` 直接 return，**用户输入被静默丢弃**（正是第 37 续修过的「列宽设置失效」同类现象）。因此**只改文案成 `%` 等于骗用户**，须连同引擎一起改（类型 + `isValidTrack` + `parseColumnWidth` + 校验 message + 测试）。另一层差异：`fr` 分配的是**扣除固定 mm 后的剩余空间**，`%` 是**相对容器总宽**，混排固定宽度时可能超出容器（溢出），需在文案/指南里提示。备选方案：保留 `fr`、文案改称「份数（填 2 表示占 2 份）」——零改动零风险但单位后缀仍是英文。**待用户拍板。**
