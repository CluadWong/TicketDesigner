...
- 当前测试基线 **vitest 210/210**，`vue-tsc --noEmit` 干净；每轮详细过程见 **[execution-log.md](./execution-log.md)**。
...
| 日期 | 节点 | 动作 | 结果 |
|---|---|---|---|
| 2026-09-02 | 删除无引用 demo 文件（廿五续） | 全项目 grep 确认 `demo*.html`（5 个）零代码引用，从 `demo/` 删除；仅保留 `云铝-*` 样例 | 全量 vitest 204/204、vue-tsc 干净 |
| 2026-09-03 | 填充数据导入/导出（三十续） | 工具栏新增「填充数据」组（导入/导出/读取/保存，`DATA_STORAGE_KEY` 与 schema 隔离）；`importFillDataFile` 进预览态播种、`exportFillDataFile`/`saveFillDataToLocal` 经 `collectFieldValues(canvasEl)` 采集导出（仅预览态）；新增 `DesignerApp.fillData.test.ts`（2 例） | 全量 vitest 210/210、vue-tsc 干净 |
| 2026-09-03 | 拖拽 `:draggable` 移出内核（三十一续） | `GridSchemaNode` 删除 6 处 `:draggable="nodeDraggable"` 与 `nodeDraggable` 计算属性（内核零拖拽属性/逻辑）；`CanvasSurface` 新增 `applyNodeDraggable()` 在 MutationObserver 周期对 `[data-node-id]:not(.layout-grid__cell)` 设 `draggable="true"`（仅 design 态）；两处快照重生成去 `draggable="true"` | 全量 vitest 210/210、vue-tsc 干净 |
| 2026-09-03 | 自动分页真实高度校正（二十续） | 渲染内核测真实行高回灌 `paginateSchema({measureRow})` 二次分页（设计/渲染态共用通道）；分页关闭态纸张改回 `min-height`（内容超高不溢出纸外）；新增 renderer「paginate=false→min-height」断言 + 引擎 measureRow 测试 | 全量 vitest 203/203、vue-tsc 干净 |
| 2026-09-03 | Table 按数据行跨页切分（廿一续） | 新增 `paginateTable()`（顶层 Table 按数据行均匀切分）+ `splitGrid` 嵌套检测（Grid 单行内含超高 Table 时递归按 Table 数据行切分，每页相同 Grid 行 + `_paginateMaxRows` 限制）；渲染层 `tablePaginatedRowCount()` 限制片段行数；`TableNodeV2._paginateMaxRows?` 内部分页字段；新增引擎测试 Table(50行) in Grid(1行) → 多物理页、总行数不丢 | 全量 vitest 204/204、vue-tsc 干净 |
| 2026-09-02 | 分页结果可观测（十九续） | 状态栏接入 `paginateSchema`：内容超高换页显示「打印：N 张」，单节点超高被裁显示「分页告警：K」；与渲染内核同函数同口径 | 全量 vitest 201/201、vue-tsc 干净 |
| 2026-09-02 | 分页收尾（十八续） | 删除旧 v1 `src/engine/` 并撤销 tsconfig/vitest 的 `exclude`；纸张 `min-height`→固定 `height`＝整纸高（同步 PaperSizePrint 断言与快照）；设计器新增「分页演示(50 行)」入口与「分页」开关（预览/打印强制分页）；Table 按行跨页切分延后 | 全量 vitest 197/197、vue-tsc 干净 |
| 2026-09-02 | 分页引擎（十六续） | 新增 `src/engine-v2/pagination.ts`（确定性分页）接入 `GridFormRenderer`（`paginate` 属性）；50 行 Grid 演示 + 预览下拉 | 全量 vitest 194/194、vue-tsc 干净 |
...
| 2026-09-02 | 删除 50 行分页演示 UI（廿二续） | 移除 `DesignerApp`「分页演示(50 行)」按钮+`loadPaginationDemo()`、`preview/App.vue` 下拉选项与 `makeFiftyRowGridSchema` 导入；保留 `src/dev/gridPaginationDemo.ts` 作测试夹具（引擎/渲染器/设计器 3 个分页测试仍复用）；删除 `DesignerApp.pagination.test.ts` 中点击 demo 按钮的用例 | 全量 vitest **203/203**、vue-tsc 干净（204→203，移除 1 例 demo 按钮用例） |
| 2026-09-02 | P11-2 完整票快照基线（廿三续） | `YunlvSecondTicketFull.test.ts` 新增整票 `toMatchSnapshot` 结构回归（608 行快照、13 段网格 + 63 字段节点全捕获）；原 4 例（校验/边框分布 all×3·outer×9·none×4/字段覆盖/表格行/段网格 id）保留 | 全量 vitest **204/204**、vue-tsc 干净 |
| 2026-09-02 | P12 清理（廿四续） | 安全项收尾：D1 死代码 grep 确证已删；修正 StatusBar.vue/gridPaginationDemo.ts 过期注释；architecture-layering-review D 类标注实际状态（D1 已解决/D3 受 A5 门控/D2 属设计层）；development-plan §16 P12 清单勾选 | 全量 vitest **204/204**、vue-tsc 干净（仅注释/字面量改动） |
...
| 2026-09-03 | A5 分层重构（二十六续，Batch 0–2） | C2 节点地址契约（node-address.ts + engine.md §18）、A1 显式 mode 取代 data!=null 推断、A4 已落地、A5 内核移除 selectedNodeId/.layout-node--selected（D3 关闭）、选中高亮迁至新建 CanvasSurface 表面层（MutationObserver）；预览态字段仍按同一 `<p>` 可编辑（canFill=mode!=="design" && !readonly）；P9.1c/P9.2 延后 | 全量 vitest **208/208**、vue-tsc 干净 |
| 2026-09-03 | A6 分层重构（二十七续，Batch 3 落点侧 + 源下沉） | 拖拽落点 `@dragover/@dragleave/@drop/@dragend` + `computeInsertionIndex` + `dragOverCellId/dragOverIndex/legalDropCellIds` 状态由 DesignerApp 整体下沉至新建 CanvasSurface 表面层；拖拽**源**（`@dragstart` 委托）亦由内核 `GridSchemaNode` 移除、改 CanvasSurface 根节点事件委托（写 NODE_MOVE_MIME + 表格内部 / 字段 p 需 Alt 拦截规则）；`DesignerApp` 退化为薄壳（仅 `moveNodeToIndexV2`/`appendNodeToCellV2` 提交）；`node-address.ts` 新增 `PALETTE_DRAG_MIME`；`:draggable` 因浏览器要求保留在内核（快照无需重生成） | 全量 vitest **208/208**、vue-tsc 干净（零回归） |
| 2026-09-03 | C1 统一编辑闸门（二十八续，Batch 5 启动） | `DesignerApp` 新增单一 `editable = !previewMode` 闸门，收敛 6 处 `if (previewMode.value) return`（`addRootGrid`/`addGrid`/`addNodeToSelectedCell`/`startPaletteDrag`/`selectNode`/`selectNodeById`）+ 模板区 6 个按钮 `:disabled`，「是否可编辑」单一来源；`previewMode` 仅保留展示语义（画布 class / mode 透传 / 选中置空 / 分页） | 全量 vitest **208/208**、vue-tsc 干净（零回归） |
| 2026-09-03 | C3 非设计态隐藏配置面板（二十九续，Batch 5 续） | 右侧 Inspector `<aside class="v2-sidebar v2-sidebar--right">` 加 `v-if="editable"`，仅设计态可见；预览/填充态该面板移出布局、画布占满，形成干净「只看表单」形态（左侧模板/结构面板保留，按钮已 `:disabled="!editable"`） | 全量 vitest **208/208**、vue-tsc 干净（零回归） |
...
| 2026-09-02 十四续 | 新增 Grid `gap` 单元格间距配置 | Schema：`GridNodeV2.gap?:number`（mm，同时作用于行/列间距，等价于 CSS `gap`）；`schema-v2-operations` 加 `resolveGridGapV2`（缺省/非法回退 0）/ `updateGridGapV2`（0/负/非数字清字段）；`schema-v2-serialization` 的 grid 分支规范化非法 gap、`schema-v2-validation.validateGrid` 加 `INVALID_GRID_GAP` warning；渲染层 `.layout-grid` 容器加 `rowGap`（flex 行间距）、`.layout-grid__row` 加 `columnGap`（列间距），导入 `resolveGridGapV2`；设计器 Grid 检查器「单元格默认」组新增「单元格间距(mm)」输入（`data-grid="gap"` → `updateGridGap`）。内部线单边归属在 gap 下表现为线间等距留白（CSS 必然结果，符合预期），相邻 Grid 外框去重不受影响；嵌套 Grid 各自独立计算 gap | vue-tsc 干净；vitest 全量 **184/184（20 文件）** 通过（177 + 新 7），无回归；未跑 `vite build`；未提交 git |
| 2026-09-02 十三续 | 修复「选 A3 横向却仍按 A4 打印、内容被裁」（真机反馈） | 根因：`@page { size: A4 }` 硬编码在 `DesignerApp.vue`（`@page` 是页面级规则，不能 scoped、不能 Vue 绑定），P11-3 只把方向派生落到渲染层与校验层，打印纸张尺寸没跟着变 → 屏幕 420mm、打印页 210mm。修复：①`types/schema-v2.ts` 新增共享 `PAPER_SIDE_MM` + `resolvePaperSizeV2(paper)`→`{size,orientation,widthMm,heightMm}`；②新增 `renderer-v2/page-size-style.ts`（`document.head` 单例 `<style>` 写 `@page { size: Wmm Hmm; margin:0 }`，`setPageSizeStyle` 幂等更新 + `registerPageSizeStyle()` 返回 release + `currentPageSizeStyle()`）；③内核 `GridFormRenderer` 用 `resolvePaperSizeV2` 并在 `watch(immediate)`/`onUnmounted` 中同步注入（消费页 `<FormRenderer>` 同步受益）；④`DesignerApp` 删硬编码 `@page`；⑤`schema-v2-validation` 删除本地 `PAPER_SIDE_MM` 副本改用共享函数；新增 `PaperSizePrint.test.ts` 4 例（A4/A3/切换/卸载清理） | vue-tsc 干净；vitest 全量 **177/177（19 文件）** 通过（173 + 新 4），无回归；未跑 `vite build`；未提交 git |
...
| 2026-09-02 十五续 | 修复 gap 下内部边框归属（真机反馈） | 根因：十四续把内部垂直分隔线画在「非首列 cell 的 `border-left`」、水平线画在「非首行 cell 的 `border-top`」；gap 是列/行之间留白，非首列 cell 的 left 边框被推到 gap 中间（距前一列内容边缘 4mm），前一列视觉无边框、与 gap 融成一体（3 列时第 2 列同理缺右边框）。用户明确指认 `.layout-grid--all > .layout-grid__row > .layout-grid__cell` 样式问题。修复（`GridSchemaNode.vue` 纯 CSS）：内部线归属翻转——「非末列 cell 画 `border-right`、`非末行 cell 画 `border-bottom`」，线落在拥有它的那个 cell 自身边缘（gap 下紧贴该列/行右/下边界，留白落外侧，列/行不再粘连）；单边归属保留（非末列画 right、末列不画，与容器外框不重叠；非末行画 bottom、末行不画），嵌套 Grid/Table 相邻仍无双边框；gap=0 视觉与旧规则一致。注释同步更新（非首行上/左 → 非末行下/右） | vue-tsc 干净；vitest 全量 **185/185（20 文件）** 通过（184 + 新 1：`GridGap.test.ts` gap=4 + border=all 下 2×2=4 cell 全渲染、列间距 4mm 生效），无回归；jsdom 不计算 scoped `border-*`，归属以 CSS 规则 + 全量回归背书；未跑 `vite build`；未提交 git |
